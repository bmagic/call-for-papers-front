...
function * createUser (action) {
  const state = yield select()
  try {
    const result = yield postUrl('http://localhost:3000/v1/user/register', { email: action.email, password: action.password }, state)
...
function * login (action) {
  const state = yield select()
  try {
    const result = yield postUrl('http://localhost:3000/v1/user/login', { email: action.email, password: action.password }, state)
...
function * getUserTalks () {
  const state = yield select()
  try {
    const result = yield getUrl('http://localhost:3000/v1/user/talks', state)
...
function * addUserTalk (action) {
  const state = yield select()
  try {
    yield postUrl('http://localhost:3000/v1/user/talks', action.talk, state)
